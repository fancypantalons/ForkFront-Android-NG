...
import com.tbd.forkfront.ui.LayoutConfiguration;
import com.tbd.forkfront.ui.StockLayoutEvaluator;
...
public class WidgetLayout extends FrameLayout {
  ...
  public void setDependencies(
      WidgetLayoutController controller, EngineCommands commands, MapInputCoordinator mapInput) {
  ...
  public void addWidget(ControlWidget widget) {
    if (!mWidgets.contains(widget)) {
      ...
      widget.setOnWidgetChangeListener(
          new ControlWidget.OnWidgetChangeListener() {
            @Override
      ...
      widget.setOnWidgetLongClickListener(
          new ControlWidget.OnWidgetLongClickListener() {
            @Override
  ...
  private void copyPref(
    ...
    editor.putString(toPrefix + suffix + "type", prefs.getString(fromPrefix + suffix + "type", ""));
    editor.putString(
        toPrefix + suffix + "label", prefs.getString(fromPrefix + suffix + "label", ""));
    editor.putString(
        toPrefix + suffix + "command", prefs.getString(fromPrefix + suffix + "command", ""));
    editor.putBoolean(
        toPrefix + suffix + "horizontal",
        prefs.getBoolean(fromPrefix + suffix + "horizontal", true));
    editor.putFloat(toPrefix + suffix + "x", prefs.getFloat(fromPrefix + suffix + "x", 0));
    editor.putFloat(toPrefix + suffix + "y", prefs.getFloat(fromPrefix + suffix + "y", 0));
    editor.putInt(toPrefix + suffix + "w", prefs.getInt(fromPrefix + suffix + "w", 200));
    editor.putInt(toPrefix + suffix + "h", prefs.getInt(fromPrefix + suffix + "h", 200));
    editor.putInt(
        toPrefix + suffix + "opacity", prefs.getInt(fromPrefix + suffix + "opacity", 191));
    editor.putInt(
        toPrefix + suffix + "font_size", prefs.getInt(fromPrefix + suffix + "font_size", 15));
    editor.putInt(toPrefix + suffix + "rows", prefs.getInt(fromPrefix + suffix + "rows", 3));
    editor.putInt(toPrefix + suffix + "columns", prefs.getInt(fromPrefix + suffix + "columns", 3));
    editor.putString(
        toPrefix + suffix + "category", prefs.getString(fromPrefix + suffix + "category", null));
    editor.putBoolean(
        toPrefix + suffix + "contextual_only",
        prefs.getBoolean(fromPrefix + suffix + "contextual_only", false));
    java.util.Set<String> pinned =
        prefs.getStringSet(fromPrefix + suffix + "pinned_commands", null);
    editor.putStringSet(toPrefix + suffix + "pinned_commands", pinned);
  }

  private void clearDraftInternal(
      android.content.SharedPreferences.Editor editor,
      android.content.SharedPreferences prefs,
      String draftPrefix,
      int count) {
  ...
  public ControlWidget createWidget(ControlWidget.WidgetData data) {
    if ("dpad".equals(data.type)) {
      ...
      if (mCommands != null && data.command != null && data.command.length() > 0) {
        ...
        btn.addOnAttachStateChangeListener(
      ...
      if (mController != null
          && mController.getMapWindow() != null
          && mController.getTileset() != null) {
        ControlWidget w =
            new MinimapWidget(getContext(), mController.getMapWindow(), mController.getTileset());
  ...
  private void loadUserLayout(
      String layoutKey, android.content.SharedPreferences prefs, String basePrefix) {
    ...
    for (int i = 0; i < count; i++) {
      ...
      java.util.Set<String> pinned =
          prefs.getStringSet(prefix + "widget_" + i + "_pinned_commands", null);
      if (pinned != null) {
  ...
  private void loadStockLayout(String layoutKey) {
    ...
    List<ControlWidget.WidgetData> stockWidgets =
        evaluator.evaluateStockLayout(mScreenId, deviceKey, layoutKey);
  ...
  /**
   * Reloads the appropriate layout for the new orientation. Called by NH_State when configuration
   * changes, not by Android framework.
   */
  ...
  /**
   * Reset the current orientation's layout to stock default. Deletes user customizations for the
