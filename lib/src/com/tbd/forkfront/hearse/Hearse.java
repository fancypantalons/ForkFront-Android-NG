...
public class Hearse {
  ...
  /**
   * bon<dungeon code><0 | role code>.<level boneid | level number> XXX case_tolerant if appropriate
   */
  ...
  public Hearse(Context context, SharedPreferences prefs, String path) {
    ...
    PATTERN =
        Pattern.compile(
            "^bon[A-Z](0|("
                + context.getResources().getString(R.string.hearseRoles)
                + "))\\.([A-Z]|\\d+)\\z",
            Pattern.CASE_INSENSITIVE);
    if (HEARSE_CRC == null) return;
  ...
  /**
   * The main method. Registers new user and uploads and downloads bones files. Originally called
   * drive() when single threaded. :)
   */
  private Thread hearseThread =
      new Thread() {
        @Override
        public void run() {
  ...
  private void updateLastUpload() {
    ...
    for (File bones : enumerateBones()) {
      ...
      if (lastModified > lastUpload) lastUpload = lastModified;
    }
  ...
  int downloadBones() throws IOException {
    ...
    while (true) {
      ...
      if (errorHeader != null) {
        ...
        // For thread safety, don't download as real name.  Nethack might try to load it before
        // complete
        File tmpBonesFile = new File(dataDirString, bonesFile.getName() + ".tmp");

        try (InputStream in = resp.getInputStream();
            BufferedOutputStream out =
                new BufferedOutputStream(new FileOutputStream(tmpBonesFile))) {
        ...
        if (checkMD5(md5Value, tmpBonesFile)) {
          if (tmpBonesFile.renameTo(bonesFile)) {
            ...
            Log.print(
                "Failed to rename bones file from "
                    + tmpBonesFile.getName()
                    + " to "
                    + bonesFile.getName());
            tmpBonesFile.delete();
          }
  ...
  int uploadBonesFiles(List<File> files) {
    ...
    for (int i = 0; i < files.size(); i++) {
      ...
      headerList.add(
          new HearseHeader(
              HEADER_VERSIONCRC, getStringMD5(info1 + "," + info2 + "," + info3 + "," + info4)));
  ...
  private NHFileInfo loadFile(File file) {
    ...
    try (DataInputStream in =
      ...
      if (mLittleEndian) buf.order(ByteOrder.LITTLE_ENDIAN);
  ...
  HearseResponse doGet(String baseUrl, String action, List<HearseHeader> headers)
      throws IOException {
  ...
  HearseResponse doPost(String baseUrl, String action, List<HearseHeader> headers, byte[] data)
      throws IOException {
    URL url = new URL(baseUrl + action);
    HttpURLConnection conn = (HttpURLConnection) url.openConnection();
    try {
