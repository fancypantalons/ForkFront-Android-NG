...
public class UpdateAssets {
  ...
  public UpdateAssets(
      AppCompatActivity activity, Listener listener, ProgressListener progressListener) {
  ...
  private static void convertFromOldPreferences(AppCompatActivity activity) {
    ...
    if (oldActivityName == null || oldActivityName.length() == 0) return;

    SharedPreferences oldPrefs =
        activity.getSharedPreferences(oldActivityName, AppCompatActivity.MODE_PRIVATE);
  ...
  private File load() {
    try {
      ...
      if (saveDir.exists() && !saveDir.isDirectory()) saveDir.delete();
  ...
  private FileStatus checkFiles(File dstPath) throws IOException {
    ...
    if (verDat == 0 || srcVer != curVer) {
      ...
      if (dst.exists() && dst.lastModified() > verDat) mBackupDefaultsFile = true;
  ...
  private File getExternalDataPath() {
    ...
    if (Environment.MEDIA_MOUNTED.equals(state)) {
      if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
        ...
        dataDir =
            new File(Environment.getExternalStorageDirectory(), "/Android/data/" + mNamespace);
      }
    }
    return dataDir;
  }
