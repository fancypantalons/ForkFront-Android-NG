 ...
 *
 * <p>Survives configuration changes (like screen rotation) and manages the NetHack engine thread,
 * game state, and JNI callbacks using Application context to avoid Activity leaks.
 */
public class NetHackViewModel extends ViewModel {
  ...
  /**
   * Initialize the ViewModel with Application context and decoder. Creates NetHackIO and NH_State
   * internally using Application context. This only happens once - on first creation.
   *
  ...
  /**
   * Attach an Activity to the ViewModel. Called when Activity is created or recreated (e.g., after
   * rotation). Processes any pending UI operations that were queued while no Activity was
   * available.
   *
  ...
  /**
   * Detach the current Activity from the ViewModel. Called when Activity is paused (e.g., going to
   * background). UI operations will be queued until a new Activity is attached.
   */
  public void detachActivity() {
    synchronized (this) {
      if (mCurrentActivity != null) {
        android.util.Log.d(
            TAG, "Detaching Activity: " + mCurrentActivity.getClass().getSimpleName());
  ...
  /**
   * Run an operation that requires an Activity context. Always posts to UI thread for thread
   * safety. If an Activity is currently attached and not finishing, posts immediately. Otherwise,
   * queues the operation until an Activity becomes available.
   *
  ...
  public void runOnActivity(Runnable operation) {
    synchronized (this) {
      ...
      android.util.Log.d(
          TAG, "Queued UI operation (queue depth: " + mPendingUIOperations.size() + ")");
    }
  }

  /**
   * Start the NetHack engine thread. Only starts once - subsequent calls are ignored.
   *
