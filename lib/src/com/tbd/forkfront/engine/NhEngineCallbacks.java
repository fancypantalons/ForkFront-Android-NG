...
import com.tbd.forkfront.window.menu.MenuSelectMode;
...
/**
 * JNI callback receiver for the NetHack engine. Every method on this class runs on the Android main
 * UI thread.
 */
public final class NhEngineCallbacks implements NH_Handler {
  ...
  public void printTile(
      int wid, int x, int y, int tile, int bkglyph, int ch, int col, int special) {
    NHW_Map map = mMapProvider.get();
    if (map != null) {
  ...
  public void cliparound(
      int x, int y, int playerX, int playerY, int objectFlags, int nearbyMonsters) {
  ...
  public void highlightDPad() {
    ...
    mScope.runOnActivity(
        () -> {
          AppCompatActivity activity = mScope.getActivity();
          if (activity != null) {
            ControlWidget existingDPad =
                WidgetLayoutController.findDpadWidget(mWidgets.getPrimaryWidgetLayout());
            if (existingDPad == null) {
              existingDPad =
                  WidgetLayoutController.findDpadWidget(mWidgets.getSecondaryWidgetLayout());
            }
