...
/**
 * Evaluates stock widget layouts from JSON into absolute screen positions. Converts anchor-based
 * positioning (LEFT/RIGHT/CENTER + margins in DP) to pixel coordinates based on actual window
 * metrics.
 */
public class StockLayoutEvaluator {
   ...
   */
  public List<ControlWidget.WidgetData> evaluateStockLayout(
      String screenId, String deviceKey, String layoutKey) {
    try {
  ...
  private InputStream openAsset(String screenId, String deviceKey, String layoutKey) {
    ...
    if (deviceKey != null) {
      try {
        return mContext
            .getAssets()
            .open("default_layouts/" + screenId + "/" + deviceKey + "_" + layoutKey + ".json");
      } catch (Exception e) {
        /* ignore */
      }
    }
  ...
  private int evaluateSize(
      String spec, boolean isWidth, int marginStart, int marginEnd, Rect bounds) {
