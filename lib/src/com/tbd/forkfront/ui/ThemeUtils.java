...
public final class ThemeUtils {
    ...
    /**
     * Resolve a color theme attribute for the given context, falling back
     * to a color resource if the attribute can't be resolved. The fallback
     * resource is read via ContextCompat, so it will respect the
     * configuration's night-mode (values-night/colors.xml wins in dark mode).
     */
    @ColorInt
    public static int resolveColor(Context context, @AttrRes int themeAttr,
                                   @ColorRes int colorResFallback) {
