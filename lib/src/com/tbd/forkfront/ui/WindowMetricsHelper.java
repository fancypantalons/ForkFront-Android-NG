...
/**
 * Helper class for obtaining window dimensions using the WindowMetrics API. Requires API 33+
 * (Android 13+).
 */
public class WindowMetricsHelper {
  ...
  /**
   * Get the current window bounds excluding system UI (status bar, navigation bar). This represents
   * the actual usable area for positioning widgets.
   *
   * @param context Context (should be an Activity context when possible)
  ...
  /**
   * Get the current window bounds excluding display cutout only. This represents the usable area
   * for immersive fullscreen layouts, where system bars are transient overlays rather than reserved
   * space.
   *
   * @param context Context (should be an Activity context when possible)
  ...
  public static Rect getLayoutBounds(Context context) {
    ...
    Insets cutout = windowInsets.getInsetsIgnoringVisibility(WindowInsets.Type.displayCutout());
