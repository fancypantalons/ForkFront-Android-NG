...
public class NH_CharacterPicker {
    ...
    mState
        ...
        .runOnActivity(
            () -> {
              ...
              mRoot =
                  (ViewGroup)
                      Util.inflate(activity, R.layout.dialog_character_picker, R.id.dlg_frame);
              ...
              mList.setAdapter(
                  new CharacterAdapter(
                      mSaves,
                      mState.getPrefs().getLastUsername(),
                      mSaveDir,
                      new CharacterAdapter.OnCharacterSelectedListener() {
                        @Override
  ...
  private void showNewCharacterPanel() {
    ...
    if (mNameInput.isInTouchMode()) {
      InputMethodManager imm =
          (InputMethodManager) mActivity.getSystemService(AppCompatActivity.INPUT_METHOD_SERVICE);
      if (imm != null) {
  ...
  public boolean handleGamepadKey(int keyCode, KeyEvent event) {
    android.util.Log.d(
        "NH_Picker", "handleGamepadKey: " + keyCode + " action: " + event.getAction());
      ...
      switch (keyCode) {
        ...
        case KeyEvent.KEYCODE_ENTER:
          ...
          if (mNewCharacterPanel != null
            ...
            InputMethodManager imm =
                (InputMethodManager)
                    mActivity.getSystemService(AppCompatActivity.INPUT_METHOD_SERVICE);
            if (imm != null) {
              imm.showSoftInput(mNameInput, InputMethodManager.SHOW_IMPLICIT);
            }
            return true;
