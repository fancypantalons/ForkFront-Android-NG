...
/**
 * A widget that displays a scrollable grid of command buttons. Can be configured with rows,
 * columns, category filter, and orientation.
 */
public class CommandPaletteWidget extends ControlWidget implements GameContextListener {
  ...
  private void populateCommands() {
    ...
    Collections.sort(
        commands,
        new Comparator<CmdRegistry.CmdInfo>() {
          @Override
    ...
    if (mHorizontal) {
      ...
      for (int i = 0; i < commands.size(); i++) {
        ...
        params.height = 0;
        params.setGravity(
            android.view.Gravity.FILL_VERTICAL | android.view.Gravity.CENTER_HORIZONTAL);
      ...
      for (int i = 0; i < commands.size(); i++) {
        ...
        params.setGravity(
            android.view.Gravity.FILL_HORIZONTAL | android.view.Gravity.CENTER_VERTICAL);
        mGridLayout.addView(btn, params);
      }
    }
  }
  ...
  public void setConfiguration(
    ...
    boolean pinsChanged =
        mPinnedCommands == null ? pinnedCommands != null : !mPinnedCommands.equals(pinnedCommands);
