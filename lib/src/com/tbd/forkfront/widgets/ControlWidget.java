...
public class ControlWidget extends FrameLayout {
  ...
  private void init(@NonNull Context context, View contentView, String type) {
    ...
    mEditPlaceholder.setTextColor(
        ThemeUtils.resolveColor(
            context, R.attr.colorOnSurfaceVariant, R.color.md_theme_onSurfaceVariant));
    mEditPlaceholder.setGravity(Gravity.CENTER);
    mEditPlaceholder.setTextSize(14);
    mEditPlaceholder.setVisibility(GONE);
    addView(
        mEditPlaceholder, new LayoutParams(LayoutParams.MATCH_PARENT, LayoutParams.MATCH_PARENT));
  ...
  public void setEditMode(boolean enabled) {
    ...
    mEditPlaceholder.setVisibility(
        enabled && mEditPlaceholder.getText().length() > 0 ? VISIBLE : GONE);
  }
          ...
          public boolean onTouch(View v, MotionEvent event) {
            ...
            switch (action) {
              case MotionEvent.ACTION_DOWN:
                ...
                if (rawX >= location[0]
                    && rawX <= location[0] + mResizeHandle.getWidth()
                    && rawY >= location[1]
                    && rawY <= location[1] + mResizeHandle.getHeight()) {
  ...
  public void applyOpacity() {
    ...
    if ("status".equals(mData.type) || "message".equals(mData.type)) {
      ...
      if (mContentView != null) {
        int surfaceColor =
            ThemeUtils.resolveColor(getContext(), R.attr.colorSurface, R.color.md_theme_surface)
                & 0x00FFFFFF;
