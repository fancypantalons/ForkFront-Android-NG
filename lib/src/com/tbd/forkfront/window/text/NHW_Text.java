...
public class NHW_Text extends AbstractNhWindow {
  ...
  public void clear() {
    mBuilder = new SpannableStringBuilder();
    if (mFragment != null && mFragment.isAdded()) mFragment.updateText();
  }
  ...
  public void show(boolean bBlocking) {
    ...
    mIsVisible = true;
    if (mFragment == null) mFragment = new NHW_TextFragment(this);
  ...
  public void scrollToEnd() {
    if (mFragment != null && mFragment.isAdded()) mFragment.scrollToEnd();
  }
  ...
  public static class NHW_TextFragment extends Fragment {
    ...
    public View onCreateView(
      ...
      ScrollView scroll =
          textView.getParent() instanceof ScrollView ? (ScrollView) textView.getParent() : null;

      if (mParent.mBuilder.length() > 0) textView.setText(mParent.mBuilder);
    ...
    void updateText() {
      ...
      if (view != null) {
        ...
        if (mParent.mBuilder.length() > 0) tv.setText(mParent.mBuilder);
    ...
    void scrollToEnd() {
      ...
      if (view != null) {
        final ScrollView scroll =
            view.findViewById(R.id.text_view).getParent() instanceof ScrollView
                ? (ScrollView) view.findViewById(R.id.text_view).getParent()
                : null;
        if (scroll != null) {
          scroll.post(
              new Runnable() {
    ...
    KeyEventResult handleKeyDown(char ch, int nhKey, int keyCode, Set<Input.Modifier> modifiers) {
      if (ch == '<') keyCode = KeyEvent.KEYCODE_PAGE_UP;
      else if (ch == '>') keyCode = KeyEvent.KEYCODE_PAGE_DOWN;
      ...
      switch (keyCode) {
        ...
        case KeyEvent.KEYCODE_VOLUME_UP:
          if (scroll != null && textView != null) scroll.scrollBy(0, -textView.getLineHeight());
          break;
        ...
        case KeyEvent.KEYCODE_PAGE_UP:
          if (scroll != null) scroll.pageScroll(View.FOCUS_UP);
          break;
        ...
        case KeyEvent.KEYCODE_PAGE_DOWN:
          if (scroll != null) {
            if (isScrolledToBottom(scroll)) mParent.close();
            else scroll.pageScroll(View.FOCUS_DOWN);
          }
          break;
      }
    ...
    private boolean isScrolledToBottom(ScrollView scroll) {
      ...
      if (count > 0) {
        ...
        if (scroll.getScrollY() + scroll.getHeight() >= view.getBottom()) return true;
      }
      return false;
    }
