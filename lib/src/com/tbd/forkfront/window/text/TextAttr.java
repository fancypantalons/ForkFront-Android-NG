...
public class TextAttr {
  ...
  public static Spanned style(String str, int attr, Context context) {
    ...
    if (attr == 0 || attr == ATTR_NONE || attr == ATTR_UNDEFINED) return style(str, attr, color);
  ...
  public static Spanned style(String str, int attr, int color) {
    ...
    if ((attr & ATTR_BOLD) != 0) span.setSpan(new StyleSpan(Typeface.BOLD), 0, str.length(), 0);

    if ((attr & ATTR_ULINE) != 0) span.setSpan(new UnderlineSpan(), 0, str.length(), 0);
