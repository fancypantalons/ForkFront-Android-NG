...
public class NHW_Map implements NH_Window {
  ...
  public NHW_Map(
    ...
    for (Tile[] row : mTiles) {
      for (int i = 0; i < row.length; i++) row[i] = new Tile();
    }
  ...
  public void setContext(AppCompatActivity context) {
    ...
    if (mUI != null) {
      ...
      if (parent != null) parent.removeView(mUI);
    }
    ...
    if (mIsVisible) show(mIsBlocking);
    else hide();
    updateZoomLimits();
  }
  ...
  public void clear() {
    synchronized (mTiles) {
      for (Tile[] row : mTiles) {
        for (int i = 0; i < row.length; i++) row[i].glyph = -1;
      }
    }
  }
  ...
  public int getTileGlyph(int x, int y) {
    if (x < 0 || x >= TileCols || y < 0 || y >= TileRows) return -1;
    synchronized (mTiles) {
  ...
  public char getTileChar(int x, int y) {
    if (x < 0 || x >= TileCols || y < 0 || y >= TileRows) return ' ';
    synchronized (mTiles) {
  ...
  public short getTileOverlay(int x, int y) {
    if (x < 0 || x >= TileCols || y < 0 || y >= TileRows) return 0;
    synchronized (mTiles) {
  ...
  public int dxFromKey(int c) {
    ...
    if (l == getDirChar(DIR_UL) || l == getDirChar(DIR_LEFT) || l == getDirChar(DIR_DL)) d = -1;
  ...
  public int dyFromKey(int c) {
    int d;
    int l = Character.toLowerCase(c);
    if (l == getDirChar(DIR_UL) || l == getDirChar(DIR_UP) || l == getDirChar(DIR_UR)) d = -1;
    else if (l == getDirChar(DIR_DL) || l == getDirChar(DIR_DOWN) || l == getDirChar(DIR_DR)) d = 1;
    else d = 0;
    if (l != c) return d * 8;
    return d;
  }
  ...
  public void preferencesUpdated(SharedPreferences prefs) {
    ...
    int borderColor =
        Color.rgb(
            red * borderOpacity / 256, green * borderOpacity / 256, blue * borderOpacity / 256);
  ...
  public void setBlocking(boolean bBlocking) {
    if (bBlocking) {
      ...
      showControls();
      if (mIsBlocking) mCommands.sendKeyCmd(' ');
      mUI.setBlockingInternal(bBlocking);
    }
  ...
  public KeyEventResult handleKeyDown(
    ...
    if (keyCode == KeyAction.ZoomIn || keyCode == KeyAction.ZoomOut) {
      ...
      if (Math.abs(mViewport.mScaleCount - scale) < 0.1 && repeatCount == 0) resetZoom();
      saveZoomLevel();
