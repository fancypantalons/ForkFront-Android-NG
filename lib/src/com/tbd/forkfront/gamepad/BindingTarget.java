...
public abstract class BindingTarget {
  ...
  public static final class NhString extends BindingTarget {
    ...
    public NhString(String line) {
      this.line = line;
    }

    @Override
    public Kind kind() {
      return Kind.NH_STRING;
    }

    @Override
    public String toString() {
      return "NhString(" + line + ")";
    }
  }
  ...
  public static final class UiAction extends BindingTarget {
    ...
    public UiAction(UiActionId id) {
      this.id = id;
    }

    @Override
    public Kind kind() {
      return Kind.UI_ACTION;
    }
  ...
  /**
   * Create a target from a command key string (from CmdRegistry or asset JSON). "#pray" →
   * NhString("#pray\n") Single char → NhKey
   */
