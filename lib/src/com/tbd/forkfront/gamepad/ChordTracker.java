 ...
 *
 * <p>Rules: - Bindings fire on press (ACTION_DOWN), not release. - Pure modifiers (buttons that
 * only appear as modifiers in chords) are swallowed silently. - Chord modifiers that have been
 * consumed are swallowed on release. - Auto-repeat re-fires the same binding (except NH_STRING
 * targets, filtered by caller).
 */
public class ChordTracker {
  ...
  /**
   * Caches the binding dispatched for each primary key so auto-repeat re-fires the exact same
   * binding even if held set changes.
   */
