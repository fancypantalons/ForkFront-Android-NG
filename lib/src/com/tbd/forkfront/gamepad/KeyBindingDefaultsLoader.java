 ...
 *
 * Asset priority: default_keybindings/{deviceKey}.json → default_keybindings/generic.json
 */
public class KeyBindingDefaultsLoader {
    ...
    /**
     * Merge any new defaults (not previously applied) into an existing binding list.
     * Skips defaults whose chord conflicts with user bindings or whose sourceCmdKey is
     * already present.
     */
    public static List<KeyBinding> mergeNewDefaults(Context ctx, String deviceKey,
                                                     List<KeyBinding> existing,
                                                     SharedPreferences prefs) {
        List<KeyBinding> defaults = loadDefaults(ctx, deviceKey);
        Set<String> alreadyApplied = new HashSet<>(
            prefs.getStringSet(PREF_DEFAULTS_APPLIED, Collections.<String>emptySet()));
