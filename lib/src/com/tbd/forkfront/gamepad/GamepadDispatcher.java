 ...
 *
 * <p>Owns the KeyBindingMap, ChordTracker, AxisNormalizer, and UiCapture stack. A single
 * process-wide instance is accessible via getInstance().
 */
public class GamepadDispatcher {
  ...
  /**
   * Injected by the active Activity so the baseline fallback can synthesize key events into the
   * view tree for focus navigation in non-GAMEPLAY contexts.
   *
   * <p>Call setSyntheticDispatcher() from onResume / onPause; whichever activity is currently in
   * the foreground owns it.
   */
  ...
  public void setEnabled(boolean enabled) {
    mOptions =
        new GamepadOptions(
            enabled,
            mOptions.leftStickMovement,
            mOptions.synthDiagonals,
            mOptions.diagonalWindowMs);
  }
  ...
  public UiContextArbiter getArbiter() {
    return mArbiter;
  }
  ...
  public boolean handleKeyEvent(KeyEvent ev, UiContext ctx) {
    ...
    if (ctx == UiContext.DIRECTION_PROMPT) {
      if (action == KeyEvent.ACTION_DOWN) {
        ...
        if (keycode == KeyEvent.KEYCODE_ESCAPE || keycode == KeyEvent.KEYCODE_BUTTON_B) {
  ...
  public boolean handleGenericMotion(MotionEvent ev, UiContext ctx) {
    ...
    mAxisNormalizer.handleGenericMotion(
        ev,
        new AxisNormalizer.EventSink() {
          @Override
   ...
   *
   * <p>When synthDiagonals is enabled, the first press is held in a short buffer for
   * diagonalWindowMs. If a second adjacent movement key arrives within the window the buffer is
   * cancelled and only the diagonal fires. Otherwise the buffer flushes and the cardinal fires
   * normally (with repeat). This prevents the extra cardinal step that the old approach produced
   * before the diagonal could activate.
   */
  private boolean handleDpadDown(int dpadKeycode, boolean isRepeat) {
    ...
    if (mOptions.synthDiagonals && !mDiagonalActive) {
      if (mBufferedDpadKey != -1 && mBufferedDpadKey != dpadKeycode) {
        ...
        if (diag != 0
            && isDpadMovementBinding(mBufferedDpadKey)
            && isDpadMovementBinding(dpadKeycode)) {
          cancelDpadBuffer();
  ...
  /**
   * Maps A→DPAD_CENTER, B→BACK, L1→PAGE_UP, R1→PAGE_DOWN for contexts without a registered
   * UiCapture (e.g. drawer open, settings navigation).
   *
   * <p>D-pad events return false so the caller can let Android focus traversal handle them via
   * super.dispatchKeyEvent. The caller is responsible for ensuring such events do not reach the
   * game engine (see ForkFront.dispatchKeyEvent).
   */
  ...
  /**
   * Find a simple (single-button, no modifiers) UI_ACTION binding for a keycode. These fire in any
   * non-OTHER/BINDING_CAPTURE context.
   */
  ...
  private static boolean isDirectionChar(char ch) {
    return ch == 'h' || ch == 'j' || ch == 'k' || ch == 'l' || ch == 'y' || ch == 'u' || ch == 'b'
        || ch == 'n';
  }
}
