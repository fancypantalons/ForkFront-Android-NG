 ...
 *
 * <p>The user holds modifier buttons (L1, R1), then presses the action button, then releases all
 * buttons. When all are released the captured chord is shown and the Confirm button is enabled.
 *
 * <p>Result is delivered via FragmentResultListener with key REQUEST_KEY. Bundle extras: "primary"
 * (int), "modifiers" (int[]).
 */
public class BindingCaptureDialogFragment extends DialogFragment {
  ...
  /**
   * Interface for checking chord conflicts against the existing binding list. Returns the display
   * name of the command already bound to the given chord, or null if no conflict exists.
   */
  public interface ConflictChecker {
    @Nullable
    String getConflictCommand(@NonNull Chord chord);
  }
  ...
  public Dialog onCreateDialog(@Nullable Bundle savedInstanceState) {
    ...
    View content = getLayoutInflater().inflate(R.layout.dialog_binding_capture, null, false);
  ...
  private void checkAndShowConflict(Chord chord) {
    ...
    if (conflictName != null) {
      ...
      if (mConfirmBtn != null) {
        ...
        mConfirmBtn.setTextColor(
            com.google.android.material.color.MaterialColors.getColor(
                requireContext(), com.google.android.material.R.attr.colorError, 0));
      }
    } else {
      clearConflictWarning();
    }
