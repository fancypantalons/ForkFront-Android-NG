...
public final class GameInputRouter {
  ...
  private final UiCapture mGameUiCapture =
      new UiCapture() {
        @Override
        public boolean handleGamepadKey(KeyEvent ev) {
          ...
          if (mGetLine != null && mGetLine.isFocused()) {
            ...
            if (ev.getAction() == KeyEvent.ACTION_DOWN) {
              KeyEventResult res =
                  mGetLine.handleKeyDown('\0', 0, ev.getKeyCode(), null, ev.getRepeatCount());
          ...
          if (mQuestion != null && mQuestion.isShowing()) {
            ...
            if (ev.getAction() == KeyEvent.ACTION_DOWN) {
              KeyEventResult res =
                  mQuestion.handleKeyDown('\0', 0, ev.getKeyCode(), null, ev.getRepeatCount());
              if (res == KeyEventResult.RETURN_TO_SYSTEM) return false;
              return res == KeyEventResult.HANDLED;
            }
            return false;
          ...
          if (top != null) {
            ...
            if (ev.getAction() == KeyEvent.ACTION_DOWN)
              return top.handleKeyDown('\0', 0, ev.getKeyCode(), null, ev.getRepeatCount())
          ...
          if (message != null && message.isMoreVisible()) {
            ...
            if (ev.getAction() == KeyEvent.ACTION_DOWN)
              return message.handleKeyDown(' ', 0, KeyEvent.KEYCODE_SPACE, null, 0)
                  == KeyEventResult.HANDLED;
          }
  ...
  public boolean handleKeyDown(
    ...
    if (ret == KeyEventResult.HANDLED) return true;
    if (ret == KeyEventResult.RETURN_TO_SYSTEM) return false;

    if (keyCode == KeyEvent.KEYCODE_BACK || keyCode == KeyEvent.KEYCODE_HOME) {
      if (mCommands.expectsDirection()) return mCommands.sendKeyCmd('\033');
    }
    if (DEBUG.runTrace() && keyCode == KeyEvent.KEYCODE_BACK) android.os.Debug.stopMethodTracing();
